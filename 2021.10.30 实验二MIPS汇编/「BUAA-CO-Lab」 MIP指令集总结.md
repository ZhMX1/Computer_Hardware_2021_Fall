...
*Posted by roife on October 5, 2020*
...
![`add` 指令说明](https://roife.github.io/img/in-post/post-buaa-co/mips-add.png)

根据编码可以把一个汇编指令翻译成机器码.

![`add` 指令编码](https://roife.github.io/img/in-post/post-buaa-co/mips-add-encode.png)
...
- 在 b 类指令中, imm 为 16 位带符号数, 而且省略了最低两位 (因为最低两位一定是 `0`), 所以能转移的指令范围为 2^{16 + 2}216+2 条, 即 256KB (跳转到更多地方则需要借助 `j`).
- 在 `j` 中, PC = PC_{31..28} \mid\mid instr_index \mid\mid 0^2*PC*=*P**C*31..28∣∣*in**s**t**r**i**n**d**e**x*∣∣02, 则转移的范围为 256MB.
...
再跳转语句中, 一般会指向标签的下一行, 即 PC \leftarrow PC + 4 + \mathtt{sign\_ext}(\mathtt{offset} \Vert 0^2)*PC*←*PC*+4+sign_ext(offset∥02).
...
![栈](https://roife.github.io/img/in-post/post-buaa-co/stack.png)
...
![附加参数](https://roife.github.io/img/in-post/post-buaa-co/additional-args-and-local-vars.png)
...
![大端与小端](https://roife.github.io/img/in-post/post-buaa-co/big-endian-and-little-endian.png)

![字符串小端存储](https://roife.github.io/img/in-post/post-buaa-co/string-little-endian.png)
...
![MIPS 内存映射](https://roife.github.io/img/in-post/post-buaa-co/mips-memory-map.png)
